...
knitr::opts_knit$set(root.dir = "C:/Users/leots/Documents/R data/twin/資料檔/")
...
# read file
pathfile <- "~/R data/twin/資料檔/11. Taipei_all.sav"
...
# descr_Ch2
t <- freq(df$Ch2, plot = FALSE) %>% round(2)
kable(t)
```
...
cor(Ch2, Ch4_attack, method = "spearman", use = "na.or.complete")
...
cor.test(Ch2, Ch4_soc, method = "spearman", continuity = F, alternative = "greater")
...
cor.test(Ch2, Ch4_attention, method = "spearman", continuity = F, alternative = "greater")
...
d <- df %>% select(Ch2, Ch4_attack_sum, 
                   Bd44, Bd43_sum, Bd45_sum, Ft7_sum) %>% 
...
# descr_Ch3
t <- freq(df$Ch3, plot = FALSE) %>% round(2)
kable(t)
```

```{r plot_Ch3, fig.width=3, fig.height=4, echo=FALSE}
d <- df %>% filter(!is.na(Ch3))
ggplot(d, aes(Ch3, y = (..count..) / sum(..count..), fill = Ch3)) + 
        # bar
        geom_bar(stat = "count", colour = "black") + 
        # text prc
        geom_text(aes(label = paste(round((..count..) / sum(..count..) * 100, 2), " %", sep = "" ),
        ...
        geom_text(aes(label = paste("n = ", ..count.., sep = ""),
                      y = (..count..) / sum(..count..)), stat= "count", vjust = -.5) +
        # labels
        labs(x = NULL, y = "百分比", title = paste("身心狀況（N = ", nrow(d), "）")) +
        # adjust scales
        scale_y_continuous(labels = scales::percent, expand = c(0, 0), limits = c(0, 1.2)) +
        # legend
        scale_fill_brewer(name = NULL, palette = "Greys", labels = levels(d$Ch3)) +
        # theme
        theme_classic() +
        # customize
        theme(legend.position = "top")
